            ...
            <div className="pr-2">
              ...
              <div className="space-y-6 mb-6">
                ...
                <div className="bg-[#F0F9FB] p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.academicInformation')}</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.studentId')}</p>
                      <p className="font-medium">{student.id}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.semester')}</p>
                      <p className="font-medium">Semester {student.semester}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.gpa')}</p>
                      <p className="font-medium">{student.gpa || safeT('scad.studentProfile.notAvailable')}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.major')}</p>
                      <p className="font-medium">{student.major || safeT('scad.studentProfile.notAvailable')}</p>
                    </div>
                ...
                {student.education && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.education')}</h3>
                    <ul className="space-y-2">
                      {student.education.map((edu, idx) => (
                        <li key={idx}>
                          ...
                          {edu.faculty && <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.faculty')}: {edu.faculty}</p>}
                          {edu.semester && <p className="text-sm text-gray-600">{safeT('scad.studentProfile.fields.semester')}: {edu.semester}</p>}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {student.skills && student.skills.length > 0 && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.skills')}</h3>
                ...
                {student.personalityTraits && student.personalityTraits.length > 0 && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.personalityTraits')}</h3>
                    <ul className="space-y-2">
                      {student.personalityTraits.map((trait, idx) => (
                        <li key={idx} className="flex justify-between">
                          <span>{trait.trait}</span>
                          <span className="text-sm text-gray-600">{safeT('scad.studentProfile.rating')}: {trait.rating}/5</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {student.jobInterests && student.jobInterests.length > 0 && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.jobInterests')}</h3>
                ...
                {student.experience && student.experience.length > 0 && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.experience')}</h3>
                    <ul className="space-y-4">
                ...
                {student.internships && student.internships.length > 0 && (
                  <div className="bg-[#F0F9FB] p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#2a5f74] mb-3">{safeT('scad.studentProfile.sections.internships')}</h3>
                    <ul className="space-y-4">

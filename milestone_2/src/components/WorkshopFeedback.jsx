...
export default function WorkshopFeedback({ isOpen, onClose, onSubmit, workshop }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // onClose(); // Comment out or remove: Do not close immediately
  };

  const handleDownloadCertificate = () => {
    // Placeholder for actual certificate download logic
    console.log("Download certificate for workshop:", workshop?.id);
    alert("Certificate download started (placeholder)!");
    // Example: Trigger a download
    // const element = document.createElement("a");
    // const file = new Blob(["Certificate Content for " + workshop?.title], { type: 'text/plain' });
    // element.href = URL.createObjectURL(file);
    // element.download = `Certificate-${workshop?.title.replace(/\s+/g, '-')}.txt`;
